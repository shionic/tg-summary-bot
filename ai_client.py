    ...
    def _build_chat_completion_payload(self, conversation_text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "Ты помощник, который суммаризирует групповые чаты. Предоставь краткое саммари основных тем и ключевых моментов обсуждения. Используй Telegram MarkdownV2 форматирование для структурирования ответа: *жирный текст* для важных моментов, _курсив_ для акцентов, списки для перечислений. Отвечай на русском языке. ВАЖНО: Экранируй специальные символы MarkdownV2: \\_ \\* \\[ \\] \\( \\) \\~ \\` \\> \\# \\+ \\- \\= \\| \\{ \\} \\. \\!"
                },
                {
                    "role": "user",
