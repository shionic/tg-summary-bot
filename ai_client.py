...
class AIClient:
    ...
    def _build_chat_completion_payload(self, conversation_text: str, is_grouped: bool = False) -> Dict[str, Any]:
        ...
        if is_grouped:
            system_content = "Ты помощник, который суммаризирует групповые чаты с несколькими тредами. Предоставь краткое саммари для каждого треда отдельно, указывая название треда. Используй HTML форматирование для структурирования ответа: <b>жирный текст</b> для важных моментов, <i>курсив</i> для акцентов, списки для перечислений. Отвечай на русском языке."
        else:
            system_content = "Ты помощник, который суммаризирует групповые чаты. Предоставь краткое саммари основных тем и ключевых моментов обсуждения. Используй HTML форматирование для структурирования ответа: <b>жирный текст</b> для важных моментов, <i>курсив</i> для акцентов, списки для перечислений. Отвечай на русском языке."
